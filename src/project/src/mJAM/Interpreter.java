...
public class Interpreter {
	...
	final static int running = 0, halted = 1, failedDataStoreFull = 2, failedInvalidCodeAddress = 3,
			...
			failedArrayIndex = 8, failedNullPointer = 9, failedMethodIndex = 10;
	...
	static void interpretProgram() {
		...
		do {
			...
			case CALL:
				addr = d + content(r);
				if (addr >= Machine.PB) {
					// primitives are static methods and have no object base on
					// the stack
					callPrimitive(addr - Machine.PB);
					CP = CP + 1;
				} else {
					// non-primitives are called as instance methods with the
					// object base at stack top
					checkSpace(2);
					...
					data[ST + 1] = CP + 1; // save caller return address in
											// callee AR
					OB = temp; // set callee OB, LB, ST
					LB = ST - 1;
					ST = ST + 2;
					CP = addr; // execution continues at addr specified in CALL
								// inst
				}
				break;
			case CALLI:
				...
				break;
			case RETURN: // d = number of method args, n = size of result (0 or
							// 1)
				addr = LB - d;
				OB = data[LB];
				CP = data[LB + 2];
				...
				ST = addr + n;
				break;
			case CALLD: // dynamic method dispatch of method with index n
						// (origin 0)
				// stack: ... object base
			...
			case HALT:
				if (n > 0) {
					// halt n > 0 --> snapshot machine state and continue
					// execution
					dump();
					CP = CP + 1;
				} else
