...
public class Test {
	public static void main(String[] args) {
		...
		Machine.emit(Op.JUMP, Reg.CB, 0); // jump to coA skipping code for
											// methods in class A

		// code for p() in A
		int label_pA = Machine.nextInstrAddr();
		/* pA */Machine.emit(Op.LOAD, Reg.OB, 0); // x at offset 0 in current
													// instance of A or its
													// subclass
		Machine.emit(Op.HALT, 4, 0, 0); // show state of stack after evaluating
										// x
		...
		/*
		 * class B extends A { int y; int p(){return x+11;} }
		 */
		int patchme_coB = Machine.nextInstrAddr();
		Machine.emit(Op.JUMP, Reg.CB, 0); // jump to coB skipping code for
											// methods in class B
		 ...
		 * a.x = 44; System.out.println(a.p()); ...
		 */
		int patchme_coC = Machine.nextInstrAddr();
		Machine.emit(Op.JUMP, Reg.CB, 0); // jump to coC skipping code for
											// methods in class B
		...
		Machine.emit(Op.LOAD, Reg.LB, 3); // addr of object "a" (will be OB in
											// p_A)
		Machine.emit(Op.CALL, Reg.CB, label_pA); // call to known method p_A
		Machine.emit(Prim.putint); // print result

		/*
		 * ... A b = new B(); b.x = 66; System.out.println(b.p()); } // end main
		 * } // end class C
		 */
		...
		Machine.emit(Op.CALLD, 0); // dynamic call, method index 0 (method p)
									// should call p_B in this case since
									// dynamic type is B
		Machine.emit(Prim.putint); // print result
		Machine.emit(Op.RETURN, 1, 0, 0); // pop one arg (instance addr), return
											// no value
		...
		/*
		 * End of class declarations generate call to main()
		 */
